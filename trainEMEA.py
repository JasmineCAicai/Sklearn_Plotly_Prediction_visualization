...
# FIXME: Read the data
df = pandas.read_csv("EMEAAO_pb.csv")
...
GT = df['TotalEMEA'].values
NGT = normalization(GT)

print(G2.shape, G3.shape, G4.shape, GT.shape)

# FIXME: 4G Prediction in Europe, Middle East, And Africa
data = np.array([Year, G2, G3, NGT])
data = data.T
train_fraction = .8
...
# FIXME: Total Prediction in Europe, Middle East, And Africa
...
fig.add_trace(go.Scatter(x=x_T, y=y_test_T, mode="lines", name="Total"),
              row=2, col=1)
fig.add_trace(go.Scatter(x=df['Season'][train_number:], y=y_T[train_number-train_number_T:]-y,
                         mode="lines", name="5G Prediction"), row=3, col=1)
fig.add_trace(go.Bar(x=[0, 1, 2, 3, 4], y=deviation_5, name="5G Deviation"),
              row=3, col=2)
fig.add_trace(go.Scatter(x=df['Season'][train_number:], y=y_test_T[train_number-train_number_T:]-y_test,
                         mode="lines", name="5G"), row=3, col=1)

fig.update_layout(title="Prediction: 4G 5G in Europe, Middle East, And Africa")
